...
- Create the plan in the `.claude/plans/` directory with filename: `YYYY-MM-DD-{descriptive-name}.md`
  - Use today's date
...
1. **Read core reference files:**
   ...
   - `.claude/context/` — background context on the user and project

2. **Explore relevant areas:**
   - If creating a command: read existing commands in `.claude/commands/`
   - If modifying outputs: explore `.claude/outputs/` structure and examples
   - If updating templates: check `.claude/reference/` for existing patterns
   - If adding scripts: review `.claude/scripts/` for conventions
