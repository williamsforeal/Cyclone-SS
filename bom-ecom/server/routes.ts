...
import { db } from "./db";
import { productCandidates, competitorIntel, trendItems, productCriteriaChecks, WINNING_CRITERIA_KEYS, uploadedMedia, jobs } from "@shared/schema";
import { eq, desc, and } from "drizzle-orm";
import { triggerN8nWebhook, dispatchScrape } from "./services/n8n";
import type { ScrapeSource, ScrapeDestination } from "@shared/types/scrape";
import { z } from "zod";
...
export async function registerRoutes(
  ...
  // ─── Unified scrape dispatch (WF10) ─────────────────────────────────
  const scrapeSourceSchema: z.ZodType<ScrapeSource> = z.union([
    z.object({
    ...
    z.object({
      ...
      url: z.string().url(),
      mode: z.enum(["scrape", "crawl", "extract"]),
      options: z
        .object({
          formats: z.array(z.enum(["markdown", "html", "json", "links"])).optional(),
  ...
  const scrapeDestinationSchema: z.ZodType<ScrapeDestination> = z.union([
    z.object({ kind: z.literal("competitor_ads"), brand_handle: z.string().min(1) }),
    z.object({ kind: z.literal("product_intel"), product_id: z.string().min(1) }),
    z.object({ kind: z.literal("trend_items"), niche: z.string().min(1) }),
    z.object({ kind: z.literal("customer_reviews"), product_id: z.string().min(1) }),
  ]);

  const scrapeRequestSchema = z.object({
    source: scrapeSourceSchema,
    destination: scrapeDestinationSchema,
    triggered_by: z.enum(["manual", "schedule", "agent"]).optional(),
  });

  app.post("/api/scrape", async (req: Request, res: Response) => {
    const parsed = scrapeRequestSchema.safeParse(req.body);
    if (!parsed.success) {
      return res.status(400).json({ error: "invalid_scrape_job", issues: parsed.error.issues });
    }
    try {
      const result = await dispatchScrape(parsed.data);
      const code = result.status === "pending" ? 202 : 502;
      return res.status(code).json(result);
    } catch (error: any) {
      console.error("Error dispatching scrape:", error.message);
      return res.status(500).json({ error: error.message });
    }
  });

  const scrapeCallbackSchema = z.object({
    job_id: z.number().int().positive(),
    status: z.enum(["success", "failed"]),
    item_count: z.number().int().nonnegative().default(0),
    airtable_record_ids: z.array(z.string()).optional(),
    errors: z.array(z.string()).optional(),
  });

  app.post("/api/scrape/callback", async (req: Request, res: Response) => {
    const token = process.env.N8N_WEBHOOK_TOKEN;
    if (token && req.headers["x-webhook-token"] !== token) {
      return res.status(401).json({ error: "invalid_token" });
    }
    const parsed = scrapeCallbackSchema.safeParse(req.body);
    if (!parsed.success) {
      return res.status(400).json({ error: "invalid_callback", issues: parsed.error.issues });
    }
    const { job_id, status, item_count, airtable_record_ids, errors } = parsed.data;
    try {
      await db
        .update(jobs)
        .set({
          status,
          output: { item_count, airtable_record_ids: airtable_record_ids ?? [] },
      ...
      console.error("Error updating scrape job:", error.message);
      return res.status(500).json({ error: error.message });
    }
  });

  app.get("/api/scrape/:jobId", async (req: Request, res: Response) => {
