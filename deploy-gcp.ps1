...
gcloud sql users set-password postgres --instance=bomb-ecom-db --password=BombEcomProd2024!
...
gcloud run deploy $SERVICE `
  ...
  --set-env-vars="DATABASE_URL=postgresql://postgres:BombEcomProd2024!@localhost/bomb_ecom?host=/cloudsql/$SQL_CONN" `
...
Write-Host "  DATABASE_URL=`"postgresql://postgres:BombEcomProd2024!@localhost/bomb_ecom?host=/cloudsql/$SQL_CONN`" npx drizzle-kit push" -ForegroundColor White
Write-Host ""
...
Write-Host "  3. DATABASE_URL=postgresql://postgres:BombEcomProd2024!@127.0.0.1:5433/bomb_ecom npx drizzle-kit push" -ForegroundColor White
Write-Host ""
